...
//              static_cast<int>(this->action == other.action) & static_cast<int>(this->modifiers ==
//              other.modifiers)
//            ) != 0;
// }
//
...
//) const {
//     return ((((std::hash<int>()(k.key) ^ std::hash<int>()(k.scancode) >> 1) << 1) ^ std::hash<int>()(k.action)
//     << 1
//             )
//             << 1) ^
...
//             if (static_cast<int>((!element->second.second) | static_cast<int>(i.action == GLFW_RELEASE)) != 0)
//             // remove elements labeled to not repeat or release
...
// }
//
// void IE::Input::Keyboard::editActions(uint16_t key, const std::function<void(GLFWwindow *)> &action, bool
// repeat) {
//     IE::Input::detail::KeyPressDescription keyPressDescription{key};
//     actionsOptions.erase(keyPressDescription);
//     actionsOptions.insert({
...
//       static_cast<IE::Input::InputEngine
//       *>(IE::Core::Core::getWindow(window)->inputEngine)->getAspect("keyboard")
