...
public:
    ...
    std::shared_ptr<T> createAspect(const std::string &t_id, IE::Core::File *t_file) {
        ...
        return std::dynamic_pointer_cast<T>(m_aspects.insert({t_id, std::make_shared<T>(t_id, t_file)}).first->second);
    }

    template<typename... Args>
    std::shared_ptr<IE::Core::Asset> createAsset(const std::string &t_id, const std::filesystem::path &t_path, Args... args) {
        return createAsset(t_id, m_filesystem.addFile(t_path), args...);
    }

    template<typename... Args>
    std::shared_ptr<IE::Core::Asset> createAsset(const std::string &t_id, IE::Core::File *t_resourceFile, Args... args) {
