...
void IECamera::update() {
	...
	projectionMatrix = {glm::perspective(glm::radians(horizontalFOV), double((*linkedRenderEngine->settings->m_currentResolution)[0]) /
																	  (*linkedRenderEngine->settings->m_currentResolution)[1], 0.01,
...
void IECamera::updateSettings() {
	aspectRatio = double((*linkedRenderEngine->settings->m_currentResolution)[0]) / (*linkedRenderEngine->settings->m_currentResolution)[1];
