...
void IE::Graphics::Buffer::createBuffer(
  ...
  size_t                     t_dataSize
    ...
    if (m_status == IE_BUFFER_STATUS_CREATED) {
        m_linkedRenderEngine->getLogger().log("Attempted to create a buffer that already exists!", IE::Core::Logger::ILLUMINATION_ENGINE_LOG_LEVEL_WARN);
        return;
    }
...
void IE::Graphics::Buffer::destroyBuffer() {
    ...
    if (m_status == IE_BUFFER_STATUS_UNINITIALIZED) {
        m_linkedRenderEngine->getLogger().log("Attempted to destroy a buffer that does not exist!", IE::Core::Logger::ILLUMINATION_ENGINE_LOG_LEVEL_WARN);
        return;
    }
