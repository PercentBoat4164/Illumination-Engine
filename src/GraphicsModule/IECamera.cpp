...
void IECamera::update() {
    ...
    projectionMatrix = {glm::perspective(glm::radians(horizontalFOV), double((*linkedRenderEngine->settings->currentResolution)[0]) / (*linkedRenderEngine->settings->currentResolution)[1], 0.01, linkedRenderEngine->settings->renderDistance)};
