...
layout(location = 1) in vec3 interpolatedWorldSpaceNormal;
...
void main() {
    ...
    vec3 diffuse = vec3(texture(diffuseTexture, fragmentTextureCoordinates)) * max(dot(interpolatedWorldSpaceNormal, lightDirection), 0.0f) * lightColor * lightIntensityAfterAttenuation;
...
//    fragmentColor = aces(vec4(1));
    fragmentColor = vec4(interpolatedWorldSpaceNormal, 1.0f);
}