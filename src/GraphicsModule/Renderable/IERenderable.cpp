...
void IERenderable::_openglLoadFromDiskToRAM() {
	...
	if ((scene == nullptr) || ((scene->mFlags & AI_SCENE_FLAGS_INCOMPLETE) != 0U) || (scene->mRootNode == nullptr)) {
		linkedRenderEngine->settings->logger.log(
				"Failed to prepare scene from file: " +
				std::string(directory + modelName) + "\t\tError: " +
				importer.GetErrorString(), IE::Core::Logger::ILLUMINATION_ENGINE_LOG_LEVEL_WARN);
	}
...
void IERenderable::_vulkanLoadFromDiskToRAM() {
	...
	if ((scene == nullptr) || ((scene->mFlags & AI_SCENE_FLAGS_INCOMPLETE) != 0U) || (scene->mRootNode == nullptr)) {
		linkedRenderEngine->settings->logger.log(
				"Failed to prepare scene from file: " +
				std::string(directory + modelName) + "\t\tError: " +
				importer.GetErrorString(), IE::Core::Logger::ILLUMINATION_ENGINE_LOG_LEVEL_ERROR);
	}
