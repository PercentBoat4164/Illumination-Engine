...
void IECommandBuffer::recordBeginRenderPass(IERenderPassBeginInfo *pRenderPassBegin, VkSubpassContents contents) {
    ...
    if (status != IE_COMMAND_BUFFER_STATE_RECORDING) {
        ...
        if (status != IE_COMMAND_BUFFER_STATE_RECORDING) {
            linkedRenderEngine->settings->logger.log(
              "Attempt to record a render pass beginning on a command buffer that is not recording!", IE::Core::Logger::ILLUMINATION_ENGINE_LOG_LEVEL_ERROR
            );
        }
    }
...
void IECommandBuffer::recordSetViewport(
  ...
  const VkViewport *pViewPorts
    ...
    if (status != IE_COMMAND_BUFFER_STATE_RECORDING) {
        ...
        if (status != IE_COMMAND_BUFFER_STATE_RECORDING) {
            linkedRenderEngine->settings->logger.log(
              "Attempt to record a viewport set on a command buffer that is not recording!", IE::Core::Logger::ILLUMINATION_ENGINE_LOG_LEVEL_ERROR
            );
        }
    }
...
void IECommandBuffer::recordSetScissor(uint32_t firstScissor, uint32_t scissorCount, const VkRect2D *pScissors) {
    ...
    if (status != IE_COMMAND_BUFFER_STATE_RECORDING) {
        ...
        if (status != IE_COMMAND_BUFFER_STATE_RECORDING) {
            linkedRenderEngine->settings->logger.log(
              "Attempt to record a scissor set on a command buffer that is not recording!", IE::Core::Logger::ILLUMINATION_ENGINE_LOG_LEVEL_ERROR
            );
        }
    }
...
void IECommandBuffer::recordEndRenderPass() {
    ...
    if (status != IE_COMMAND_BUFFER_STATE_RECORDING) {
        ...
        if (status != IE_COMMAND_BUFFER_STATE_RECORDING) {
            linkedRenderEngine->settings->logger.log(
              "Attempt to record a render pass ending on a command buffer that is not recording!", IE::Core::Logger::ILLUMINATION_ENGINE_LOG_LEVEL_ERROR
            );
        }
    }
