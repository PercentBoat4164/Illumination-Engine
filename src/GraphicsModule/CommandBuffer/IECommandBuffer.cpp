...
void IECommandBuffer::recordBindVertexBuffers(uint32_t firstBinding, uint32_t bindingCount, const std::vector<IEBuffer *> &buffers,
...
void
IECommandBuffer::recordBindVertexBuffers(uint32_t firstBinding, uint32_t bindingCount, const std::vector<IEBuffer *> &buffers, VkDeviceSize *pOffsets,
										 VkDeviceSize *pSizes, VkDeviceSize *pStrides) {
	VkBuffer pVkBuffers[buffers.size()];
	for (int i = 0; i < buffers.size(); ++i) {
		pVkBuffers[i] = buffers[i]->buffer;
		addDependency(buffers[i]);
...
void IECommandBuffer::recordBindDescriptorSets(VkPipelineBindPoint pipelineBindPoint, IEPipeline *pipeline, uint32_t firstSet,
											   const std::vector<IEDescriptorSet *> &descriptorSets, std::vector<uint32_t> dynamicOffsets) {
	addDependency(pipeline);
