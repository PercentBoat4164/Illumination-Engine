...
void IE::Graphics::detail::ImageOpenGL::setLocation(IE::Graphics::Image::Location t_location) {
	...
	if (!m_data.empty() ||
		m_id != 0 && t_location != m_location) {  // If data already exists somewhere and the new location is not the same as the old
...
bool IE::Graphics::detail::ImageOpenGL::_createImage(const IE::Core::MultiDimensionalVector<unsigned char> &t_data) {
	...
	if (t_data.m_dimensions.size() > 3 || t_data.m_dimensions.empty()) {
...
void IE::Graphics::detail::ImageOpenGL::_getImageData(IE::Core::MultiDimensionalVector<unsigned char> *t_pData) const {
	std::unique_lock<std::mutex> lock(*m_mutex);
	t_pData->m_dimensions = m_dimensions;
	t_pData->m_data.resize(std::accumulate(m_dimensions.begin(), m_dimensions.end(), 1, std::multiplies()));
	glBindTexture(m_type, m_id);
	glGetTexImage(m_type, 0, m_format, GL_UNSIGNED_BYTE, t_pData->m_data.data());
	glBindTexture(m_type, 0);
}

void IE::Graphics::detail::ImageOpenGL::_setImageData(const IE::Core::MultiDimensionalVector<unsigned char> &t_data) {
	std::unique_lock<std::mutex> lock(*m_mutex);
	if (std::accumulate(m_dimensions.begin(), m_dimensions.end(), 1, std::multiplies()) != t_data.m_data.size()) {
		lock.release();
		_destroyImage();
		_createImage(t_data);
		lock.lock();
	...
	glBindTexture(m_type, m_id);
	if (m_data.m_dimensions.size() == 1) {
		glTexImage1D(GL_TEXTURE_1D, 0, m_format, (GLsizei) t_data.m_dimensions[0], 0, m_components > 3 ? GL_RGBA : GL_RGB, GL_UNSIGNED_BYTE,
					 t_data.m_data.data());
	} else if (m_data.m_dimensions.size() == 2) {
		glTexImage2D(GL_TEXTURE_2D, 0, m_format, (GLsizei) t_data.m_dimensions[0], (GLsizei) t_data.m_dimensions[1], 0,
					 m_components > 3 ? GL_RGBA : GL_RGB, GL_UNSIGNED_BYTE, t_data.m_data.data());
	} else if (m_data.m_dimensions.size() == 3) {
		glTexImage3D(GL_TEXTURE_3D, 0, m_format, (GLsizei) t_data.m_dimensions[0], (GLsizei) t_data.m_dimensions[1], (GLsizei) t_data.m_dimensions[2],
