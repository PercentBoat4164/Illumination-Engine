...
VkSurfaceKHR IERenderEngine::createWindowSurface() {
	if (glfwCreateWindowSurface(instance.instance, window, nullptr, &surface) != VK_SUCCESS) {
        settings->logger.log("Failed to create window surface!", IE::Core::Logger::ILLUMINATION_ENGINE_LOG_LEVEL_ERROR);
	}
...
IERenderEngine::IERenderEngine(IESettings *settings) {
	...
	camera.create(this);
    settings->logger.log(device.physical_device.properties.deviceName, IE::Core::Logger::ILLUMINATION_ENGINE_LOG_LEVEL_INFO);
...
bool IERenderEngine::_vulkanUpdate() {
	...
	if (frameTime > 1.0 / 30.0) {
        settings->logger.log(
                "Frame #" + std::to_string(frameNumber) + " took " + std::to_string(frameTime * 1000) +
                "ms to compute.", IE::Core::Logger::ILLUMINATION_ENGINE_LOG_LEVEL_WARN);
	}
	auto currentTime = (float) glfwGetTime();
	frameTime = currentTime - previousTime;
	previousTime = currentTime;
...
//	glfwWindowHint(GLFW_OPENGL_PROFILE, GLFW_OPENGL_CORE_PROFILE);
	...
	camera.create(this);
    this->settings->logger.log(reinterpret_cast<const char *>(glGetString(GL_RENDERER)),
                               IE::Core::Logger::ILLUMINATION_ENGINE_LOG_LEVEL_INFO);
    this->settings->logger.log(API.name + " v" + API.version.name, IE::Core::Logger::ILLUMINATION_ENGINE_LOG_LEVEL_INFO);
}
