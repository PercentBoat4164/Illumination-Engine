    ...
    for (unsigned long i = 0; i < bindings.size(); ++i) {
        if (newData[i].has_value()) {
            ...
            if (writeDescriptorSet.descriptorType == VK_DESCRIPTOR_TYPE_STORAGE_IMAGE) {
                ...
                storageBufferDescriptorInfo.buffer = std::get<OLDBuffer *>(newData[i].value())->m_buffer;
                ...
                uniformBufferDescriptorInfo.buffer = std::get<OLDBuffer *>(newData[i].value())->m_buffer;
