#include "AudioModule/AudioEngine.hpp"
...
int main(int argc, char **argv) {
    ...
    auto                    *audioEngine = IE::Core::Core::createEngine<IE::Audio::AudioEngine>("audio engine");
    std::vector<std::string> devices{IE::Audio::AudioEngine::listAudioDevices()};
    if (devices.empty()) std::cout << "Unable to gather devices from current driver!";
    for (auto &device : devices) std::cout << device << std::endl;

    auto sound =
      IE::Audio::Sound(IE::Core::Core::getFileSystem()->getFile("res/sounds/MinecraftFullSoundtrack.opus"));
